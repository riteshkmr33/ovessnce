...
class phpVimeo
{
    ...
    public function __construct($consumer_key, $consumer_secret, $token = null, $token_secret = null)
    {
        ...
        $this->session = new Container('vimeo');
    }
    ...
    private function _request($method, $call_params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true, $use_auth_header = true)
    {
        ...
        if (!empty($method)) {
            ...
            if ($response->stat == 'ok') {
				...
				$this->session->msg = $response->err->msg;
    ...
    public function upload($file_path, $use_multiple_chunks = false, $chunk_temp_dir = '.', $size = 2097152, $replace_id = null)
    {
        ...
        if ($quota->user->upload_space->free < $file_size) {
			$this->session->msg = 'The file is larger than the user\'s remaining quota '.$quota->user->upload_space->free.'.';
			return false;
        ...
        if ($file_size > $rsp->ticket->max_file_size) {
			$this->session->msg = 'File exceeds maximum allowed size.';
			return false;
        ...
        if ($use_multiple_chunks) {
            if (!is_writeable($chunk_temp_dir)) {
				$this->session->msg = 'Could not write chunks. Make sure the specified folder has write access.';
				return false;
        ...
        if ($complete->stat == 'ok') {
			...
			$this->session->msg = $complete->err->msg;
			return false;
