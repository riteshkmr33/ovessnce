...
class LS_userlogin
{
	...
	public static function lsCheckuserdata($username, $pass)
	{
		...
		global $lsdb;
		$lsdb->query('SELECT id FROM '.DB_PREFIX.'jrc_user WHERE LOWER(username) = "'.strtolower($username).'" AND password = "'.$passcrypt.'" AND access = 1');
	...
	public static function lsLogin($name, $pass, $remember)
	{
		...
		$sql = 'UPDATE '.DB_PREFIX.'jrc_user SET session = "'.smartsql(session_id()).'", idhash = "'.smartsql($nidhash).'", available = 1, forgot = IF (forgot != 0, 0, 0), lastactivity = "'.time().'" WHERE username = "'.$name.'" AND password = "'.$passcrypt.'"';
	...
	public static function lsConfirmidhash($username, $idhash)
	{
		...
		if (isset($username)) {
		    ...
		    $sql = 'SELECT idhash FROM '.DB_PREFIX.'jrc_user WHERE LOWER(username) = "'.smartsql(strtolower($username)).'" AND access = 1';
	...
	public static function lsUserinfo($username)
	{
			...
			global $lsdb;
			$sql = 'SELECT * FROM '.DB_PREFIX.'jrc_user WHERE LOWER(username) = "'.smartsql(strtolower($username)).'" AND access = 1';
			$result = $lsdb->queryRow($sql);
	...
	public static function lsUpdatelastactivity($userid)
	{
			...
			global $lsdb;
			$lsdb->query('UPDATE '.DB_PREFIX.'jrc_user SET lastactivity = "'.time().'" WHERE id = "'.smartsql($userid).'"');
	...
	public static function lsForgotpassword($email, $time)
	{
			...
			global $lsdb;
			$lsdb->query('SELECT id FROM '.DB_PREFIX.'jrc_user WHERE email="'.smartsql($email).'" AND access = 1 LIMIT 1');
			if ($lsdb->affected_rows > 0) {
				if ($time != 0) {
				$lsdb->query('UPDATE '.DB_PREFIX.'jrc_user SET forgot = "'.smartsql($time).'" WHERE email="'.smartsql($email).'"');
				}
			    return true;
			} else {
			    return false;
	...
	public static function lsForgotactive($forgotid)
	{
			...
			global $lsdb;
			$lsdb->query('SELECT id FROM '.DB_PREFIX.'jrc_user WHERE forgot = "'.smartsql($forgotid).'" AND access = 1 LIMIT 1');
			if ($lsdb->affected_rows > 0) {
			    return true;
			} else
			    return false;
	...
	public static function lsWriteloginlog($username, $url, $ip, $agent, $success)
	{
			...
			if ($success == 1) {
				...
				$lsdb->query('UPDATE '.DB_PREFIX.'jrc_loginlog SET access = 1 WHERE ip = "'.smartsql($ip).'" AND time = NOW()');
			} else {
			
				$lsdb->query('INSERT INTO '.DB_PREFIX.'jrc_loginlog SET name = "'.smartsql($username).'", fromwhere = "'.smartsql($url).'", ip = "'.smartsql($ip).'", usragent = "'.smartsql($agent).'", time = NOW(), access = 0');
			}
	...
	public static function lsLogout($userid)
	{
			...
			$lsdb->query('UPDATE '.DB_PREFIX.'jrc_user SET session = NULL, idhash = NULL, available = 0 WHERE id = "'.$userid.'"');
