...
if ($_SERVER["REQUEST_METHOD"] == 'POST' && isset($_POST['newP'])) {
    ...
    $result2 = $lsdb->query('UPDATE '.DB_PREFIX.'jrc_user SET password = "'.$passcrypt.'", forgot = 0 WHERE email = "'.smartsql($femail).'" AND forgot = "'.smartsql($page1).'"');
	
	$result = $lsdb->query('SELECT username FROM '.DB_PREFIX.'jrc_user WHERE email = "'.smartsql($femail).'" LIMIT 1');
