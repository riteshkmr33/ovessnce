		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
 				...
 				for (i in valueData){
 					...
 					if (arr.showValues){
 						...
 						fs = 12; // font-size is 0 if showValues = false
                                        ...
                                        heig = (heig > 0)?heig:NaN; // added be developer to solve the bar height issue
			...
			//creating legend array from lbl if there is no legends param
 			...
 			// animated apearing
 			if(arr.animate){
 				//$('#graphFieldBar'+unique).css({ 'display' : 'none', 'height' : 0 });  // Commented by Developer
 		...
 		if(arr.legend){
			$(el).append("<div id='legendHolder"+unique+"'></div>");
	 		$('#legendHolder'+unique).css({ 'width': legendWidth, 'float': 'right', 'text-align' : 'left', 'font-size': 11});
