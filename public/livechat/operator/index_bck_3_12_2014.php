...
if ($page && $page != '404') $_SESSION['LSRedirect'] = $_SERVER['REQUEST_URI'];
...
if (LS_USERID_RHINO) $USER_LANGUAGE = $lsuser->getVar("language");
...
// home
    if ($page == '') {
        ...
        if (!LS_USERID_RHINO) {
            ...
            $cacheverfile=APP_PATH.LS_CACHE_DIRECTORY.'/version.php';$admincacheexist=false;if(file_exists($cacheverfile)){include_once $cacheverfile;$admincacheexist=true;$admin_cache_expire=LS_ADMIN_CACHE_CREATED+86400;if($admin_cache_expire<time()){if(file_exists($cacheverfile)){unlink($cacheverfile);$admincacheexist=false;}}}if(!$admincacheexist && function_exists('simplexml_load_file')){$versioncont="<?php\n";$versioncont.="define('LS_ADMIN_CACHE_CREATED','".time()."');\n";$urlxml=parse_url(BASE_URL_ORIG);$jakXML=@simplexml_load_file('http://vc.livesupportrhino.com/rhinosocket.xml?url='.$urlxml["host"].'&on='.LS_O_NUMBER);if($jakXML){$versioncont.="define('LS_VC_STATUS',1);\n";foreach($jakXML->children()as$child){$versioncont.="define('LS_VC_".strtoupper($child->getName())."','".$child."');\n";}}else{$jakXML=jak_load_xml_from_url('http://vc.livesupportrhino.com/rhinosocket.xml?url='.$urlxml["host"].'&on='.LS_O_NUMBER);if($jakcurlxml){$versioncont.="define('LS_VC_STATUS',1);\n";foreach($jakXML->children()as$child){$versioncont.="define('LS_VC_".strtoupper($child->getName())."','".$child."');\n";}}else{$versioncont.="define('LS_VC_STATUS',0);\n";}}$versioncont.="?>";LS_base::lsWriteinCache($cacheverfile,$versioncont,'');if(file_exists($cacheverfile))include_once $cacheverfile;}
   ...
   if ($page == 'logout') {
    ...
    if ($page == 'forgot') {
    	if (LS_USERID_RHINO || !is_numeric($page1) || !$lsuserlogin->lsForgotactive($page1)) ls_redirect(BASE_URL);
