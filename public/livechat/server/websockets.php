...
if (!file_exists($final_url.'config.php')) die('socketserver.php] config.php not exist');
require_once $final_url.'config.php';
    ...
    while(true) {
      ...
      foreach ($read as $socket) {
        if ($socket == $this->master) {
          ...
          $numBytes = @socket_recv($socket,$buffer,$this->maxBufferSize,MSG_DONTWAIT); 
          if ($numBytes === false) {
          	if (is_array($client)) {
	          	$key_user_removal = array_search($socket, $client);
	          		if ($key_user_removal === false || $key_user_removal === '') {
	          	    	$this->disconnect($socket);
	          	    	$this->stdout("Client removed (array_search). TCP connection lost: " . $socket);
	          	    } else {
	          	        unset($tab_utile[$key_user_removal]);
	          	    }
	            $this->stdout('Socket error: ' . socket_strerror(socket_last_error($socket)));
	        }
          } elseif ($numBytes == 0) {
            $this->disconnect($socket);
            $this->stdout("Client disconnected (bytes 0). TCP connection lost: " . $socket);
          } else {
