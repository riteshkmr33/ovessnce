...
if (is_numeric($_GET["opid"]) && $_GET["opid"] != 0) $op_direct = $_GET["opid"];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['start_chat'])) {
		...
		if (LS_EMAIL_BLOCK && (LS_CLIENT_EMAIL || !empty($defaults['email']))) {
		...
		if ((LS_CLIENT_EMAIL || !empty($defaults['email'])) && !filter_var($defaults['email'], FILTER_VALIDATE_EMAIL)) {
		...
		if (LS_CLIENT_QUESTION && (empty($defaults['question']) || strlen(trim($defaults['question'])) <= 5)) {
		    $errors['question'] = $tl['error']['e'];
		}
		...
		if (count($errors) > 0) {
			...
			if(!empty($defaults['email'])) {
				$_SESSION['jrc_email'] = filter_var($defaults['email'], FILTER_SANITIZE_EMAIL);
			} else {
				$_SESSION['jrc_email'] = $tl['general']['g12'];
			}
				
			// Department
			if (isset($defaults['department']) && is_numeric($defaults['department'])) {
				$depsql = smartsql($defaults['department']);
			} else {
				$depsql = 1;
			}
			
			if (isset($defaults['opdirect']) && is_numeric($defaults['opdirect'])) {
				$opdirect = smartsql($defaults['opdirect']);
			} else {
				$opdirect = 0;
			}
			...
			if ($result) {
				...
				if ($defaults["question"]) {
					$lsdb->query('INSERT INTO '.DB_PREFIX.'transcript SET 
					name = "'.smartsql($_SESSION['jrc_name']).'",
					message = "'.smartsql($defaults["question"]).'",
					convid = "'.$cid.'",
					time = NOW(),
					class = "user"');
				}
				...
				$result = $lsdb->query('SELECT username, email FROM '.DB_PREFIX.'user WHERE access = 1 AND available = 0 AND emailnot = 1 AND (departments = 0 OR FIND_IN_SET('.$depsql.', departments)) AND FIND_IN_SET("'.date("D").'", tw_days) AND (TIME(NOW()) >= tw_time_from AND TIME(NOW()) <= tw_time_to)');
...
<?php if ($_GET['slide'] == 1 || $page1 == 1) { include_once('template/slide_up/start.php'); } else { include_once('template/pop_up/start.php'); } ?>
