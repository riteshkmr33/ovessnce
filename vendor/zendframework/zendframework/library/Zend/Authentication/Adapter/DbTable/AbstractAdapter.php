...
abstract class AbstractAdapter extends BaseAdapter
{
    ...
    protected function authenticateValidateResultSet(array $resultIdentities)
    {
		...
		/* set authenticated user id and user type id */
        $this->authenticateResultInfo['user_id'] = $resultIdentities[0]['id'];
		$this->authenticateResultInfo['user_type_id'] = $resultIdentities[0]['user_type_id'];
		...
		if (count($resultIdentities) < 1) {
            ...
            return $this->authenticateCreateAuthResult();
        } elseif ( $this->authenticateResultInfo['user_id'] != 1 ) {
			
			$this->authenticateResultInfo['code']       = AuthenticationResult::FAILURE_IDENTITY_NOT_FOUND;
            $this->authenticateResultInfo['messages'][] = 'Only super admin can log into this.';
            return $this->authenticateCreateAuthResult();
    ...
    protected function authenticateCreateAuthResult()
    {
        return new AuthenticationResult(
            ...
            $this->authenticateResultInfo['user_id'],
            $this->authenticateResultInfo['user_type_id'],
